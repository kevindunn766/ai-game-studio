# Procedural isopod (pill-bug / woodlouse) — 14 legs, procedurally constructed body
# and procedurally animated legs via 2-bone IK + a metachronal-wave gait.
# First creature model; exercises the construction workflow and the leg IK.
...
func _process(delta: float) -> void:
	_angle += ang_speed * delta
	_place_on_circle()
	var speed: float = abs(ang_speed) * circle_radius
