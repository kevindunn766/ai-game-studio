...
## Movement speed proportional to scale (third piece of feedback)

`Flame.move_speed` was a flat `3.0 m/s` constant regardless of `scale_factor`. At match-scale (camera view ~0.7m per `camera_controller.gd`), that's fast enough to cross the entire visible world in under a quarter second — reads as "zipping around" rather than movement proportional to a tiny creature's size. Fixed by deriving top speed every physics tick from `CameraController.target_size_for_scale(scale_factor)` instead of a constant: `speed = target_size_for_scale(scale_factor) / view_crossing_seconds` (`view_crossing_seconds = 2.2`). Since speed is now tied to the *same formula* that drives camera framing, the flame crosses roughly the same fraction of its own visible world per second at every scale — verified via a headless sweep printing `view / speed` at both ends of all 9 bands: exactly `2.2` every time, from match-scale (0.32 m/s) to Band 9's endpoint (254.8 m/s). Acceleration (`accel_ramp_time`, time-to-reach-full-speed) is kept as a flat constant on purpose, *not* scale-derived, so controls feel equally responsive at every size even though the speed they ramp to varies by three orders of magnitude.
...
## Fuel outlives its growth value (spawn window vs. value window split)

Kevin's next piece of feedback: "the grass should continue to spawn everywhere. At some scale it just doesn't give points." Previously, `_should_spawn_detail()`'s `active_while_scale_below`/`active_scale_range` fields did double duty — the same threshold controlled both whether a fuel type kept spawning *and* whether it was worth anything, so grass both vanished from the world and stopped giving points at the exact same instant (roughly the moment you left Band 1). That reads as an abrupt double-cutoff rather than "you've outgrown this."

Split into two separate, independently-tunable windows per tier in `prop_manager.gd`'s `PROP_TIERS`/`STRUCTURE_FUEL_TIERS`:
- `active_while_scale_below`/`active_scale_range` (existing fields, widened ~4-5x) — still governs `_should_spawn_detail()`, i.e. whether new instances keep appearing in the world at all.
- `value_while_scale_below`/`value_scale_range` (new fields, set to the *old*, tighter thresholds) — governs a new `_has_growth_value(tier, current_scale)` check, evaluated at ignite time (not spawn time — a fuel object that's been sitting around while the flame kept growing correctly reflects "too big for this to matter now," not whatever scale happened to be current when it spawned) in `_on_fuel_ignited()`/`_on_structure_fully_burned()`. If false, the fuel still ignites and burns down visually (`mark_burned()` still runs, `Fuel.ignite()` still plays), it just skips the `growth_controller.register_burn()` call.

Verified live: at scale 0.25 (well past `dry_grass`'s value cutoff of 0.1, but inside its new spawn window of 0.5), 4,460 grass instances were still actively spawning, one was walked into and confirmed to ignite, and `GrowthController._grow_amount` was unchanged before/after — zero points, but the world didn't go bare.
