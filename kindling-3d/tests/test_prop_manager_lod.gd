...
func _test_lod_cutoff_flips() -> void:
	...
	_assert(pm._should_spawn_detail(tier, 0.099), "grass should still spawn just under its cutoff")
	_assert(not pm._should_spawn_detail(tier, 0.1), "grass should stop spawning at its cutoff")
	_assert(not pm._should_spawn_detail(tier, 0.6), "grass should stay stopped well above its cutoff")
...
func _test_lod_range_gates_band2_tiers() -> void:
	...
	_assert(not pm._should_spawn_detail(tier, 0.6), "small_plant should not spawn past its range")
...
func _test_lod_range_gates_band3_tiers() -> void:
	...
	_assert(not pm._should_spawn_detail(tier, 5.0), "brush_pile should not spawn well past its range")
